...
async def submit_policy(
    ...
    plan_type: str = Form(...),
    plan_price: str = Form(...),
    dependents: str = Form("[]")  # JSON string
):
    ...
    try:
        ...
        return JSONResponse(
            status_code=400,
            content={"error": "Dependents must be valid JSON."}
        )
...
for dep in dependents_list:
    ...
    if not relation or not dep_dob:
        return JSONResponse(
            status_code=400,
            content={"error": "Each dependent must have relation and dob."}
        )
    ...
    if dep_age is None:
        return JSONResponse(
            status_code=400,
            content={"error": f"Invalid DOB for dependent ({relation})."}
        )
    ...
    # ---------------- CHILD RULE ----------------
        ...
        # Disabled child allowed regardless of age
        if is_disabled:
            continue
        ...
        # 22–25 must be student
        if 22 <= dep_age <= 25:
            if not is_student:
                return JSONResponse(
                    status_code=400,
                    content={"error": "Child aged 22–25 must be a student."}
                )
            continue

        # Over 25 and not disabled
        return JSONResponse(
            status_code=400,
            content={"error": "Child over 25 must be disabled to qualify."}
        )

    else:
        return JSONResponse(
            status_code=400,
            content={"error": "Invalid dependent relation type. Use Spouse or Child."}
        )
    # ---------------- PREMIUM CLEANING ----------------
